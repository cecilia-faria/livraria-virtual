    ...
    document.querySelectorAll('.botao-carrinho').forEach((botao, index) => {
        botao.addEventListener('click', () => {
            ...
            if (carrinhoProdutos[index]) {
                ...
                carrinhoProdutos[index] = { descricao, preco, img: imgSrc, quantidade: 1 };
            }
    ...
    for (const id in carrinhoProdutos) {
        ...
        divItem.innerHTML = `
            ...
            <div class="item-carrinho-info">
                ...
                <p class="preco">${formatarPreco(item.preco)} x ${item.quantidade} = ${formatarPreco(subtotal)}</p>
...
// Configura botões + e - de quantidade no carrinho
...
// Alterna visibilidade do carrinho lateral
...
function configurarBotaoFinalizarCompra() {
    btnFinalizar.addEventListener('click', () => {
        alert('Finalizando compra... (página não implementada)');
    });
}
...
function atualizarCarrossel() {
    carrosselEl.style.transform = `translateX(-${indiceAtual * 100}%)`;
}
